...
5. **需要接手人类已登录的 Chrome、复杂 iframe、多 tab 调试吗？**
   是 → 用 `Codex-in-chrome` MCP（工具前缀 `mcp__claude-in-chrome__*`），参考 `refs/playwriter.md`
...
| 已登录 Chrome、iframe-heavy、手工接管 | `Codex-in-chrome` | `Codex-in-chrome` | ✅ 已接入 — Chrome 扩展管理，无需手动启动 |
...
| `refs/playwriter.md` | `Codex-in-chrome` | 已登录 Chrome / iframe-heavy / 多 tab（实际用 `mcp__claude-in-chrome__*` 工具） |
