...
"""EchoTrend 240 V2 - Signal Logic (BTC Regime Gate + V6 Execution, CAP1.0)

BTC 4H SMA240 regime gate is the core alpha source.
V6 execution package (cooldown/EMA/reentry) for execution rhythm only.
Bull regime: 100% MSTR. Bear regime: 0% MSTR. No leverage.
...
from strategies.echotrend_240_v2.indicators import (
...
from strategies.echotrend_240_v2.engine import EchoTrendEngine, EngineBacktestResult
...
def _compute_realized_return(trades: list[Trade], result, initial_capital: float) -> float:
    """Compute realized return excluding unrealized P&L from open position.

    Uses equity curve: finds the last sell event and takes equity at that point
    as the realized equity. If no sells, realized = 0.
    """
    if not trades:
        return 0.0
