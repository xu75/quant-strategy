...
"""EchoTrend 240 V2 Strategy Engine — v9 trend_regime variant (CAP1.0).

BTC 4H SMA240 regime gate is core alpha. V6 execution package for rhythm only.
Bull regime: 100% MSTR. Bear regime: 0%. No leverage (max_exposure=1.0).
"""
...
class EchoTrendEngine:
    ...
    def _execute_symmetric(
        ...
        # CAP1.0: regime flip → immediate full execution (no step fractions)
        is_regime_flip = (target_exp <= 0.001 and current_exp > 0.5) or \
                         (target_exp >= 0.999 and current_exp < 0.5)
